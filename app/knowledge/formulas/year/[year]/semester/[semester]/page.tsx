...
export default async function FormulaSemesterPage({
  params,
}: {
  params: Promise<{
    year: string;
  ...
  return (
    <div>
      ...
      <p className="mb-10 text-slate-400">
        Select a subject to browse formula sheets.
      </p>
      ...
      {subjects.length === 0 ? (
        ...
        <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
          {subjects.map((subject) => (
            <Link
              key={subject.id}
              href={`/knowledge/formulas/year/${yearNumber}/semester/${semesterNumber}/subject/${encodeURIComponent(
                subject.name
              )}`}
              className="rounded-3xl border border-slate-800 bg-[#111827] p-7 transition hover:-translate-y-1 hover:border-emerald-500"
            >
